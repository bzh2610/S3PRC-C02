...
#~~~~ adjust platform-specific features (Posix/Windows/Emscripen...) ~~~~
ifeq (${OS},Windows_NT)
  LIB_PREFIX=
  LIB_SUFFIX=.dll
  EXE_SUFFIX=.exe
...
#~~~~ compiler message colorisation (if available) ~~~~
COLOR_CC=${strip ${firstword ${wildcard ./color_cc.py ../color_cc.py \
                                        /home/TP/sujets/Tools/color_cc.py}}}
ifneq (${COLOR_CC},)
  CC:=python ${COLOR_CC} ${CC}
  CXX:=python ${COLOR_CC} ${CXX}
  LD:=python ${COLOR_CC} ${LD}
endif
...
${LIB_TARGET} : ${COMMON_OBJECT_FILES}
	@echo ==== linking $@ ====
	${LD} -shared -o $@ $^ ${CPPFLAGS} ${CFLAGS} ${LDFLAGS}
	@${SKIP_LINE}
